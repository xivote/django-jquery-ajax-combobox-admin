/**
 * @modified_by: Sergio Ruiz Bens <sergio.ruizbens@gmail.com>
 * @changes: CHANGES.md
*/
 ...
 * @param {string} [option.base_query=''] - '' (e.g.: "user__login=admin")
 ...
 * @param {string} [option.paging_bar='bottom'] - 'bottom', 'top'
 ...
 * @param {string} [option.loading_img='']
...
$.extend($.ajaxComboBox.prototype, /** @lends external:jQuery.ajaxComboBox.prototype */ {
  ...
  _setOption2nd: function(option) {
    ...
    option.base_query = (option.base_query) ? option.base_query : '';
  ...
  _setInitRecord: function() {
    ...
    if (typeof this.option.source == 'object') {
      ...
      var self = this;
      if (self.option.loading_img != '')
          $(self.elem.img).attr('src', self.option.loading_img);
      $.ajax({
        dataType: 'json',
        url: self.option.source,
        data: {
          db_table: this.option.db_table,
          base_query: this.option.base_query,
        ...
        success: function (json) {
          if (self.option.loading_img != '')
              $(self.elem.img).attr('src', self.option.button_img);
          self._afterInit(self, json);
        },
        error: function(jqXHR, textStatus, errorThrown) {if (self.option.loading_img != '') {$(self.elem.img).attr('src', self.option.button_img);} self._ajaxErrorNotify(self, errorThrown); }
      });
    }
  },
  ...
  _searchForDb: function(self, q_word, which_page_num) {
    if (self.option.loading_img != '')
        $(this.elem.img).attr('src', self.option.loading_img);
    self.prop.xhr = $.ajax({
      dataType: 'json',
      url: self.option.source,
      data: {
        ...
        base_query: self.option.base_query,
        field: self.option.field,
        show_field: self.option.show_field,
        ...
        for (i = 0; i < json.cnt_page; i++) {
          ...
          for (var key in json.result[i]) {
            ...
            if (key == self.option.field) {
              ...
              if ($.inArray(key, self.option.show_field) != -1)
      ...
      complete: function() {if (self.option.loading_img != ''){$(self.elem.img).attr('src', self.option.button_img);} self.prop.xhr = null; }
    });
  },
  ...
  _displayResults: function(self, arr_candidate, arr_subinfo, arr_primary_key) {
    ...
    for (var i = 0; i < arr_candidate.length; i++) {
      ...
      if (arr_subinfo) {
        ...
        //show fields order by self.option.show_field
        sorted_fields_list = []
        if (self.option.show_field[0] != "*") {
            for (index in self.option.show_field)
                sorted_fields_list.push(self.option.show_field[index]);
        }
        else {
            for (var key in arr_subinfo[i])
                sorted_fields_list.push(key);
        }

        for (var index in sorted_fields_list) {
           key = sorted_fields_list[index];
          // sub_info属性の値を整える
